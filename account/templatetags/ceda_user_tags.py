...
try:
    from dj_security_middleware.utils.request import (
        login_url as legacy_login,
        logout_url as legacy_logout,
    )
...
_use_legacy_login = _legacy_login_loaded and LEGACY_MIDDLEWARE in settings.MIDDLEWARE
...
def logout_with_post():
    ...
    return settings.LOGOUT_WITH_POST if hasattr(settings, "LOGOUT_WITH_POST") else True
...
def logout_url(context):
    ...
    name = (
        settings.LOGOUT_URL_NAME
        if hasattr(settings, "LOGOUT_URL_NAME")
